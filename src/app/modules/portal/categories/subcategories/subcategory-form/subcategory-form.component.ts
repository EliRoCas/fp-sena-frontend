...
export class SubcategoryFormComponent {
  ...
  constructor(
    ...
    effect(() => {
      if (this.id) {
        this.store
          .select(subcatById(this.id!))
          .pipe(filter((subcatData) => !!subcatData))
  ...
  add() {
    if (this.id) {
      this.store.dispatch(
        subcatAdapter.patchOne(
          this.subcatForm.value as unknown as SubcategoryModel,
          (data) => {
