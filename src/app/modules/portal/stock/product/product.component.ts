...
export class ProductComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.sub = this.route.params.subscribe((params) => {
      ...
      if (this.id) {
        this.store
          .select(productById(this.id!))
          .pipe(filter((productData) => !!productData))
  ...
  add() {
    if (this.id) {
      this.store.dispatch(
        productAdapter.patchOne(
          this.productForm.value as unknown as ProductModel,
          (data) => {
