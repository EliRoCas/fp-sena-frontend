...
export class StockComponent {
  ...
  ngOnInit(): void {
    ...
    this.store.select(selectProductsWithCategories).subscribe(data => this.dataSource.set(data));


    this.store.select(productAdapter.selectById("2")).subscribe(data => this.selected.set(data));

    this.store.select(productByName('')).subscribe(data => this.dataSource.set(data));
  };
