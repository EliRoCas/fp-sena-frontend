...
export const productAdapter = new EntityAdapter<ProductModel>('products', {
  getId: (input) => input.id_product.toString(),
  additionalData: { id: 'id_product' },
});

export const productByName = (name: string) =>
  createSelector(productAdapter.feature, (products) => {
    return products.filter(
      (p) => p.product_name.toLowerCase().indexOf(name.toLocaleLowerCase()) > -1
    );
  });
...
export type ProductCategory = ProductModel & { category_name: string };
  ...
  (products: ProductModel[], categories: CategoryModel[]) => {
    return products.map((product) => {
      const category = categories.find(
        (cat) => cat.id_category === product.fo_category
      );
      return {
        ...product,
