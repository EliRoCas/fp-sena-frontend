describe('Caso de Prueba No. 06 - Módulo de Administración', () => {
  it('Debería generar y actualizar el reporte de Estado de Resultados correctamente', () => {
    ...
    cy.get('a[routerLink="/portal/income-statement"]').click();

    

    // Verificar que el reporte se genere correctamente y contenga la información adecuada
    cy.get('table').should('be.visible');
    cy.get('table').contains('Ingreso Neto').should('be.visible');
    cy.get('table').contains('Ingresos Totales').should('be.visible');
    cy.get('table').contains('Total de Gastos Operativos').should('be.visible');
    ...
    cy.url().should('include', '/portal/report');
       cy.get('a[routerLink="/portal/income-statement"]').click();
    cy.url().should('include', '/portal/income-statement');
